...
namespace KmeansTest
{
    class KMeans
    {
        public static float[][] Cluster2(float[][] rawData, int numClusters, out int[] clustering, bool isNormalized = true)
        {
            // k-means clustering
            ...
            //    update means
            // end loop
            clustering = InitClustering(data.Length, numClusters, 0); // semi-random initialization
            float[][] means = InitMeans(data, numClusters); // small convenience
            ...
            Console.WriteLine("SSE: {0}", SSE(data, clustering, means));
            return means;
        }

        public static float[][] Cluster(float[][] rawData, int numClusters,out int[] clustering,  bool isNormalized = true)
        {
            // k-means clustering
            // index of return is tuple ID, cell is cluster ID
            // ex: [2 1 0 0 2 2] means tuple 0 is cluster 2, tuple 1 is cluster 1, tuple 2 is cluster 0, tuple 3 is cluster 0, etc.
            ...
            // end loop
            clustering = InitClustering(data.Length, numClusters, 0); // semi-random initialization
            float[][]means = Allocate(numClusters, data[0].Length); // small convenience
        ...
        private static  float SSE(float[][] data, int[] clustering, float[][] means)
        {
            float result = 0.0f;
            for (int i = 0; i < data.Length; i++)
            {
                float dist = Distance(data[i], means[clustering[i]]);
                result += (float)Math.Pow(dist, 2.0);
            }
        ...
        private static float[][] InitMeans(float[][] data, int numClusters)
        {
            ...
            int[] meansIndex = new int[numClusters];
            ...
            for (int i = 0; i < numClusters; i++)
            {
                ...
                if (!isDuplicated)
                {
                    means[i] = new float[data[index].Length];
                    Array.Copy(data[index], means[i], data[index].Length);
                }
                else
                {
                    i--;
